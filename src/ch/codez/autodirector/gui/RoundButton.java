...
public class RoundButton extends JButton {
    ...
    private final static int CIRCLE_BORDER = 40;
    ...
    private void init() {
        ...
        if (icon != null) {
            ...
            size.width = size.height = Math.max(icon.getIconWidth() + CIRCLE_BORDER, icon
                    .getIconHeight()
                    + CIRCLE_BORDER);
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (this.getIcon() != null) {
            this.getIcon().paintIcon(this, g, CIRCLE_BORDER / 2 - 1, CIRCLE_BORDER / 2 - 1);
        }
    }
    ...
    protected void paintBorder(Graphics g) {
        ...
        // inner circle
        Arc2D.Double arc = new Arc2D.Double(BORDER_DIST, BORDER_DIST, getSize().width - 2 - 2
                * BORDER_DIST, getSize().height - 2 - 2 * BORDER_DIST, 0, 360, Arc2D.CHORD);
        GradientPaint gp = new GradientPaint(pt1, col2, pt2, col1, true);
        g2.setPaint(gp);
        g2.draw(arc);

        // outer circle
        arc = new Arc2D.Double(0, 0, getSize().width - 2, getSize().height - 2, 0, 360, Arc2D.CHORD);
    ...
    private void paintHighlight(Graphics g) {
        ...
        Arc2D.Double arc = new Arc2D.Double(1, 1, getSize().width - 3, getSize().height - 3, 0,
                360, Arc2D.CHORD);
        g2.fill(arc);
    }
